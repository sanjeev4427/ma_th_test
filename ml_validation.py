...
def  apply_best_sett_exp_one_hy(sbj, args,*arg):  
    ...
    mod_val_preds, mod_val_preds_copy, mod_val_data_saved,mod_val_comp_saved = data_skipping_all_act(np.copy(val_output[:,0]), config, data_saved, computations_saved, apply_best, best_filename)
    ...
    # np.savetxt(mod_val_val_pred_file_path, mod_val_val_pred, delimiter=',')
    ...
    mod_val_comp_saved = skip_count_act/(skip_count_act + threshold_count_act)*100
    mod_val_data_saved = np.clip(100*(skip_count_act - lam*(skip_count_act+1))/(skip_count_act - lam*(skip_count_act+1) + threshold_count_act + (threshold_count_act-1)*(1 - lam)), a_min=0, a_max=None)
...
def ml_validation(args, data, algo_name, log_folder_name):
    ...
    #         if args.name:
    #             mod_cp_score_acc.to_csv(
    #                 os.path.join(
    #                     log_dir,
    #                     f'mod_cp_scores_acc_{args.dataset}_{args.algo_name}_{args.name}_seed_{seed_i}.csv',
    #                 )
    #             )
