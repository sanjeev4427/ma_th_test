...
from GA_model_exp_gt_data.genetic_algo_gt_data import genetic_algorithm_gt_data
            ...
            best_h_param, loss, best_f1, f_one_target, best_comp_saved, best_data_saved,\
                f_one_gt_mod_val,  f_one_gt_val, f_one_val_mod_val,\
                        f_one_gt_mod_val_avg, f_one_gt_val_avg=\
                                genetic_algorithm_gt_data(activity, args, ml_train_gt_pred, n_bits,  termin_iter, max_iter, n_pop, r_cross, r_mut, log_dir, sbj, labels, filename_best_csv, activity_name)
            # get the end time
            end_time = time.time()
...
def ga_activity_wise_exp_gt(args, data, bounds, n_bits,  n_pop, r_cross, r_mut, termin_iter, max_iter, log_dir, *arg):
    ...
    # for sbj in [0]:
        ...
        for labels in activity_labels:
            ...
            # setting tol = 0
            bounds = [[1, args.max_win_thr], [1, args.max_win_skip], [0,0]]
            ...
            best_h_param, loss, best_f1, f_one_target, best_comp_saved, best_data_saved,\
                f_one_gt_mod_val,  f_one_gt_val, f_one_val_mod_val,\
                        f_one_gt_mod_val_avg, f_one_gt_val_avg=\
                                genetic_algorithm(activity, activity_name, args, ml_train_gt_gt, bounds, n_bits,  termin_iter, max_iter, n_pop, r_cross, r_mut, log_dir, sbj)
            # get the end time
            end_time = time.time()
            elapsed_time = end_time - start_time
        ...
        train_tolerance_hyp(args, data, n_bits,  n_pop, r_cross, r_mut, termin_iter, max_iter, log_dir, sbj, filename_best_csv, exp_name)
